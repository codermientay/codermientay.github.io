...
#             utils.saveimg(generated_image, "output.jpg")

def stylize(style_path, input_img_path, output_img_path):
    device = ("cuda" if torch.cuda.is_available() else "cpu")
    net = transformer.TransformerNetwork()
    net.load_state_dict(torch.load(style_path, map_location=torch.device('cpu')))
    net = net.to(device)
    with torch.no_grad():
        content_image = utils.load_image(input_img_path)
        if content_image is None:
            raise ValueError(f"Không load được ảnh đầu vào: {input_img_path}")
        content_tensor = utils.itot(content_image).to(device)
        generated_tensor = net(content_tensor)
        generated_image = utils.ttoi(generated_tensor.detach())
        if PRESERVE_COLOR:
            generated_image = utils.transfer_color(content_image, generated_image)
        utils.saveimg(generated_image, output_img_path)
